...
/** JSON-safe DTO for RSC → client (`TradeErpSyncPanel`). Dates are ISO-8601. */
export type HotSalesSyncJobDetailDto = {
  id: string;
  jobType: HotSalesSyncJobType;
  entityId: string;
  idempotencyKey: string;
...
export function toHotSalesSyncJobDetailDto(
  job: HotSalesSyncJobDetail,
): HotSalesSyncJobDetailDto {
  return {
    id: job.id,
    jobType: job.jobType,
