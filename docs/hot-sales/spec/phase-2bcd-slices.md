...
| **Phase 2A baseline** | Tag `hot-sales-phase-2a` → `8e650ff` · ops Gates 1–7 closed |
...
1. **Feature flags** — `HOT_SALES_DEPOSIT_ENABLED`, `HOT_SALES_PICKUP_OPS_ENABLED`, `HOT_SALES_NOTIFICATIONS_ENABLED`, `HOT_SALES_ERP_SYNC_ENABLED` (manifest + accessors). Default `false` until slice acceptance + prod promotion checklist.
2. **Additive schema only** — No destructive changes to `013_hot_sales.sql` / `014_hot_sales_rbac.sql` tables.
...
4. **Mail boundary** — Hot Sales notifications use dedicated provider (ADR-003); never Neon Auth shared mail.
5. **Commit lane** — Hot Sales 2B–2D commits stay isolated from declaration portal, Guardian Auth, PA, repo normalization.
...
| | |
|--|--|
| **Goal** | Migration `015_hot_sales_deposit.sql`; domain services for deposit CRUD, adjustments, audit |
| **AC** | Tables per ADR-002; `deposit_status` projection updater; L0 tests; `deposit.manage` / `deposit.view` enforced |
| **Flag** | `HOT_SALES_DEPOSIT_ENABLED` |
| **Depends on** | 2B-0 |
...
| | |
|--|--|
| **Goal** | Finance/ops UI under `/trade/[locale]/admin` — record receipt, waive, refund, forfeit |
...
| | |
|--|--|
| **Goal** | Migration `016_hot_sales_pickup_ops.sql`; windows, assignments, fulfillment, exceptions |
| **AC** | State machine per ADR-002; `fulfilled_quantity` rollup; `pickup.manage` / `pickup.view` enforced; L0 tests |
| **Flag** | `HOT_SALES_PICKUP_OPS_ENABLED` |
| **Depends on** | 2B-2 |
...
**2B closure:** Tag `hot-sales-phase-2b` after 2B-4 + smoke — checklist in [ops/gate-register.md](../ops/gate-register.md) § Phase 2B closure tag.
...
| | |
|--|--|
| **Goal** | `hot_sales_import_batch` / `import_row`; dry-run + confirm; templates for priority + supply |
...
| **Flag** | `HOT_SALES_NOTIFICATIONS_ENABLED` |
| **Depends on** | 2C-1 |
...
**2C closure:** Tag `hot-sales-phase-2c` after 2C-4 + notification smoke — checklist in [ops/gate-register.md](../ops/gate-register.md) § Phase 2C closure tag.
...
| **AC** | Integration spec in `docs/hot-sales/integrations/<customer>.md` (or generic stub) |
| **Depends on** | 2C-4 |
...
| **AC** | No sync in HTTP request path; idempotency on push; `HOT_SALES_ERP_SYNC_ENABLED` default false |
| **Depends on** | 2D-0 |
...
| | |
|--|--|
| **Goal** | Vendor-specific pack under `lib/domain/trade/erp/<vendor>/` |
...
**2D closure:** Tag `hot-sales-phase-2d` after 2D-2 minimum (2D-3 per customer project).
...
|-------|------|-------------|-----|
| 2B | `lib/domain/trade/deposit*`, `pickup*` | Admin forms | Extend `e2e/trade-hot-sales.spec.ts` |
