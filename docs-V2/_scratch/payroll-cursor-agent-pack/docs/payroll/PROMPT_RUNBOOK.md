# Cursor prompt runbook for @afenda/payroll

Use one prompt per phase. Review the plan and diff before proceeding. Do not ask Agent to build the entire package in one task.
...
We are introducing packages/erp/payroll with package name @afenda/payroll. Do not edit files yet.

Inspect the repository and identify canonical examples for:
...
Produce docs/payroll/REPOSITORY_ALIGNMENT.md containing:
...
```text
Read @docs/payroll/REPOSITORY_ALIGNMENT.md, @docs/payroll/IMPLEMENTATION_PLAN.md, and the applicable .cursor rules.
...
Implement the listed payroll permissions, stable error codes, PayrollEmployeeQueryPort, infrastructure ports discovered in the repository, and intentional public exports. Do not implement calculations or database tables yet.
...
Explain which suggested tables can be deferred from the first migration and why. Confirm no foreign key or direct mutation crosses into HR, Payments or Accounting ownership. Save the reviewed design to docs/payroll/SCHEMA_DESIGN.md.
```

## Prompt 03 — Persistence foundation

```text
Implement only the reviewed first migration and store foundation from @docs/payroll/SCHEMA_DESIGN.md.

Create the store contract, memory store, Drizzle store and resolver following canonical repository patterns. Keep store methods aggregate-oriented rather than exposing generic table mutation.
