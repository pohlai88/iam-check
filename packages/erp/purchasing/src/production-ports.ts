	...
	return {
		async record(input: AuditFactInput): Promise<Result<{ id: string }>> {
			try {
				...
				return failFromUnknown(error, "Failed to write purchasing audit fact");
			}
		},
	};
}
...
export function createSqlOutboxPort(): OutboxPort {
	return {
		async append(input: OutboxFactInput): Promise<Result<{ id: string }>> {
			...
			if (!parsed.success) {
				return fail("BAD_REQUEST", "Invalid purchasing outbox event", {
			...
			try {
				...
				return failFromUnknown(
					error,
					"Failed to append purchasing outbox event",
				);
			}
		},
	};
