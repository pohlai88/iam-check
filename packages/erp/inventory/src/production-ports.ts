	...
	return {
		async record(input: AuditFactInput): Promise<Result<{ id: string }>> {
			try {
				...
				return failFromUnknown(error, "Failed to write inventory audit fact");
			}
		},
	};
}
...
export function createSqlOutboxPort(): OutboxPort {
	return {
		async append(input: OutboxFactInput): Promise<Result<{ id: string }>> {
			...
			if (!parsed.success) {
				return fail("BAD_REQUEST", "Invalid inventory outbox event", {
			...
			try {
				...
				return failFromUnknown(
					error,
					"Failed to append inventory outbox event",
				);
			}
		},
	};
