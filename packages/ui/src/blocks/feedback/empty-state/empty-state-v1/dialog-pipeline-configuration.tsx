...
const InitializePipelineDialog = ({
	defaultOpen = false,
	trigger,
	className,
}: Props) => {
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			>
								...
								{steps.map((step) => (
									<div key={step.number}>
										...
										<Select
											...
											onValueChange={(val) =>
												val &&
												setValues((prev) => ({ ...prev, [step.number]: val }))
											}
										>
				...
				<div className="bg-background sticky bottom-0 z-10 flex shrink-0 flex-col-reverse items-center gap-4 border-t px-6 py-4 sm:flex-row sm:justify-end">
					<DialogClose
						render={<Button variant="outline" className="max-sm:w-full" />}
					>
						Cancel
					</DialogClose>
					<DialogClose render={<Button className="max-sm:w-full" />}>
						Initialize
					</DialogClose>
				</div>
			</DialogContent>
