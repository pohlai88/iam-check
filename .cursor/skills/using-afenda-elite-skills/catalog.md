...
| `afenda-elite-ui-compose` | local-elite-farm | local | keep | Product UI consistency lock — barrel compose, type/spacing/radius/color tables, anti-handroll; Vitest compose-redflags | Compose product UI · fix handroll · visual consistency | ADR-010 · ARCH-024 · tokens.css · apps/web Geist map | none | `.cursor/skills/afenda-elite-ui-compose/SKILL.md` | Done = `pnpm check:ui-system` green; load before frontend-ui-engineering method; companion reference.md |
