...
| `afenda-elite-ui-compose` | local-elite-farm | local | keep | Product UI consistency lock — barrel compose, type/spacing/radius/color tables, anti-handroll | Compose product UI · fix handroll · visual consistency | ADR-010 · ARCH-024 · tokens.css · apps/web Geist map | none | `.cursor/skills/afenda-elite-ui-compose/SKILL.md` | Consistency only — not beauty; load before frontend-ui-engineering method; companion reference.md |
