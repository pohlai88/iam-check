...
function main() {
  ...
  const expected = CANONICAL_VERCEL_KEYS.filter(
  ...
  console.log(
    `Present on Vercel: ${[...remote].filter((k) => expected.includes(k)).length}`
  );
  ...
  console.log(
    "Tenant-owned syncOptional keys (e.g. FFT_ERP_VENDOR) are not required until ERP sync is enabled."
  );
}
